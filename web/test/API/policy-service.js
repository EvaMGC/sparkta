describe('API.policy-service', function() {
  ...
  beforeEach(module('api/policyError.json'));


  var srv, rootScope, httpBackend, fakePolicyById, fakePolicyList, fakePoliciesStatusList, fakePolicyError = null;

  beforeEach(
      inject(function($httpBackend, $rootScope, _ApiPolicyService_, _apiPolicy_, _apiPolicyList_, _apiPoliciesStatusList_, _apiPolicyError_) {
        fakePolicyById = angular.copy(_apiPolicy_);
        fakePolicyList = _apiPolicyList_;
        fakePoliciesStatusList = _apiPoliciesStatusList_;
        fakePolicyError = _apiPolicyError_;
  ...
  it("Should return the last error of a  policy by its Id", function() {
    var policyId = '2581f20a-av83-4315-be45-192bc5sEdFff';
    httpBackend.when('GET', '/policy/error/' + policyId).respond(fakePolicyError);

    srv.getLastError().get({id: policyId}).$promise.then(function(result) {
      expect(JSON.stringify(result)).toEqual(JSON.stringify(fakePolicyError));
    });
